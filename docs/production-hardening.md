...
- release approver 先用 `/api/v1/admin/promotion/preflights` 生成 fresh deep+ops go-live preflight，再用 `/api/v1/admin/promotion/decisions` 记录 approve/reject/defer、target version、备注和绑定的 preflight id；preflight id 单次消费，blocked gate 只能通过显式 override 审计，preflight 后证据 drift 必须重跑。
