...
2. 再做 retry：只重试 `retryable=true` 且副作用安全的工具；HTTP business adapter 和 HTTP knowledge adapter 已内置有限重试，写工具只有带幂等键才会重试。
3. 再做 circuit breaker：连续 `429`、`5xx`、timeout、network failure、transient bad JSON 会打开业务或知识库上游断路器，readiness 会显示 `circuit=open`。
